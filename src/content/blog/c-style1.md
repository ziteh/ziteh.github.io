...
4. 16 進制數的數值部分的英文**必須**為大寫，如：`0x3A`、`0xFF8D`。
...
2. 參數（Parameter）
   1. 名稱**必須**使用**蛇形**風格，命名**應該**偏好**簡潔有力**，為名詞。
...
1. 全域範圍
    ...
    1. `const` 全域常數名稱**必須**使用**大寫蛇形**風格，命名**應該**偏好**具描述性**，為名詞。
2. 區域變數名稱**必須**使用**蛇形**風格，命名**應該**偏好**簡潔有力**，為名詞。
...
1. 列舉（Enum）、結構（Struct）與聯合（Union）
    ...
    2. 成員的名稱**必須**使用**蛇形**風格，命名**應該**偏好**簡潔有力**，為名詞。
...
4. 總是**必須**明確地加上 `default`，即使它的內容為空。
...
在可讀性上巨集也差很多。多行的 `#define` 要在行末加上 `\`，複雜的甚至還要使用 `do-while(0)` 包圍才可以確保編譯器將其解釋為一個 Block。而 `inline` 函數就只是比一般的函數前面多一個修飾詞而已。當然，如果這個巨集足夠單純的話，使用巨集無可厚非，但是還是要注意巨集的非預期行為。

巨集可能在展開後出現非預期行為。想像一下如何用巨集實現一個 `MAX(a, b)` 函數，它會比較兩個參數並回傳較大的那個。感覺很簡單？

```c
#define MAX(a, b) (a) > (b) ? (a) : (b)
```

實際上上面的寫法在某些情況下會有非預期行為，這是比較實務的寫法（[Linux 核心原始程式碼巨集: max, min](https://hackmd.io/@sysprog/linux-macro-minmax)）：
...
那如果是其它預處理器判斷式呢？我認為，如果那些判斷式寫的很複雜或很多行（尤其無法在一個螢幕上顯示）的話，可以考慮加上註解。但同時也要優先考慮是否真的有必要使用這麼複雜或長的預處理器判斷式？
